...
function Header() {
  const input = document.querySelector('.header__input');
    ...
    useEffect(()=>{
      if (surch.surch) {
        getfilms(surch.surch).then(data=>setFilm(data));
      }
  },[surch.surch])
  return (
    <nav className="header">
      <div className="header__container">
        ...
        <div class="header__inputContainer">
            ...
            <input className="header__input" type="text" name="surch" value={surch.surch} onChange={FilmChange} ref={inputRef}/> 
        </div>
      </div>
      <CardMovie surch={surch} Film={Film}></CardMovie>
    </nav>
